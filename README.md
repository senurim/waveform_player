# waveform_player
라즈베리파이를 이용하여 특정 주파수의 6가지 파형을 3.5mm 오디오 잭을 통해 출력하는 파이썬 프로그램입니다. 이 프로그램 구현은 SoX 프로그램(play)을 이용하여 간단하게 만들었고, CLI 방법으로 실행시킵니다.

이와 다르게 푸시 버튼 스위치를 라즈베리파리의 GPIO 핀에 연결하여 디폴트 주파수를 가지는 6가지 파형을 버튼을 누를 때마다 차례대로 출력하는 별도의 `gpio_waveform_player.py` 코드도 만들었습니다.
